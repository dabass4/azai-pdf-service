...
**Option 2: Via API**
```bash
curl -X PUT "https://timesheet-claims.preview.emergentagent.com/api/patients/{patient_id}" \
...
**Option 2: Via API**
```bash
curl -X PUT "https://timesheet-claims.preview.emergentagent.com/api/employees/{employee_id}" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "first_name": "John",
...
curl "https://timesheet-claims.preview.emergentagent.com/api/patients?is_complete=false" \
  -H "Authorization: Bearer YOUR_TOKEN"

curl "https://timesheet-claims.preview.emergentagent.com/api/employees?is_complete=false" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
4. **Test Connection**
   ...
   curl "https://timesheet-claims.preview.emergentagent.com/api/sandata/test-connection" \
     -H "Authorization: Bearer YOUR_TOKEN"
   ```
